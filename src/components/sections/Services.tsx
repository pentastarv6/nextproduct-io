...
export default function Services({ t }: ServicesProps) {
  return (
    <section id="services" className="relative bg-dark py-24 px-6" aria-labelledby="services-heading">
      {/* Background accent */}
      <div className="pointer-events-none absolute top-0 left-1/2 -translate-x-1/2 h-px w-3/4 bg-gradient-to-r from-transparent via-brand-500/40 to-transparent" aria-hidden="true" />

      <div className="mx-auto max-w-7xl">
        {/* Header */}
        <div className="mb-16 text-center">
          <p className="section-label mb-3">{t.services.label}</p>
          <h2 id="services-heading" className="text-3xl font-display font-bold text-white sm:text-4xl lg:text-5xl">
            {t.services.title}
          </h2>
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {t.services.items.map((item, i) => (
            <article key={item.title} className="card group">
              <div className="mb-4 inline-flex h-12 w-12 items-center justify-center rounded-xl bg-brand-500/15 text-brand-400 transition-all duration-300 group-hover:bg-brand-500/25 group-hover:text-brand-300">
                {serviceIcons[i]}
              </div>
